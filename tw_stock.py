...
try:
    ...
    exchange_dict.update({
        ...
        '5274': 'TPEX', '5371': 'TPEX', '8261': 'TPEX', '5314': 'TPEX', '7795': 'TPEX'
    })
    ...
    # 強制萃取 4 位數股號，並精準配對 TWSE 或 TPEX
    def build_tw_ticker(raw_text):
        match = re.search(r'(\d{4})', str(raw_text)) 
        if match:
            code = match.group(1)
            ex = exchange_dict.get(code, "TWSE") 
            return f"{ex}:{code}"
        return str(raw_text)

    df['ticker'] = pd.Series(df.index.values).apply(build_tw_ticker).values
